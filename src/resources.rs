...
impl Resources {
    ...
    pub fn load_cstring(&self, resource_name: &str) -> Result<ffi::CString, Error> {
        ...
        )?; // Note that ? here will transform io::Error into Error::Io(io::Error) using the From trait

        // Allocate buffer the same size as the file
...
// Transform resource path names like 'shaders/triangle.vert' into OS paths, for portability
fn resource_name_to_path(root_dir: &Path, location: &str) -> PathBuf {
    let mut path: PathBuf = root_dir.into(); // Path into PathBuf
